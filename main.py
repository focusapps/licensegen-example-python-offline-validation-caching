...
def verify_response_signature(sig_header, digest_header, date_header, response_body):
  ...
  signing_data = inspect.cleandoc(
    ...
    """.format(
      account_id=os.environ['LICENSEGEN_ACCOUNT_ID'],
      date=date_header,
  ...
  verify_key = ed25519.VerifyingKey(os.environ['LICENSEGEN_VERIFY_KEY'].encode(), encoding='hex')
...
def validate_license_key(key):
  ...
  try:
    res = requests.post(
      'https://licensegen-api.focusapps.app/v1/accounts/{}/licenses/actions/validate-key'.format(os.environ['LICENSEGEN_ACCOUNT_ID']),
      headers={
  ...
  # by pinging https://google.com, or https://licensegen-api.focusapps.app/v1/ping.
